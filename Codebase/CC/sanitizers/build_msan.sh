...
docker run \
  ...
  bash -ec '
    mkdir -p $BINTAR_DIR
    cd $BINTAR_DIR
    cmake -DWITH_MSAN=ON -DCMAKE_EXE_LINKER_FLAGS="-L${MSAN_LIBDIR} -Wl,-rpath,${MSAN_LIBDIR}" -DCMAKE_MODULE_LINKER_FLAGS="-L${MSAN_LIBDIR} -Wl,-rpath,${MSAN_LIBDIR}" -DCMAKE_SHARED_LINKER_FLAGS="-L${MSAN_LIBDIR} -Wl,-rpath,${MSAN_LIBDIR}"  -DCMAKE_C_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=RelWithDebInfo -DWITH_DOCS=ON -DPACKAGE_PLATFORM_SUFFIX=$HOSTNAME $SOURCE_DIR
