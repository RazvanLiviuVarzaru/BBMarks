...
export GIT_COMMIT=c506daee10faf6f9a8c397b5fe67a4966591e61b
...
MYSQL_TEST_TLS=0
MYSQL_TEST_SSL_PORT=
MARIADB_TLS_ONLY_TEST_ENV_VAR=1
MARIADB_TLS_DEBUG_ENV_OVERRIDE=1
MARIADB_TLS_DEBUG_OPENSSL=1
...
# -e MARIADB_ROOT_PASSWORD=$MYSQL_TEST_PASSWD \
# Sidecar
docker run \
...
docker run \
  ...
  bash -ec '
    mkdir -p $BINTAR_DIR
    cd $BINTAR_DIR
    cmake -DCMAKE_C_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=RelWithDebInfo -DWITH_DOCS=ON -DPACKAGE_PLATFORM_SUFFIX=$HOSTNAME $SOURCE_DIR
...
docker run \
  -e BINTAR_DIR=$BINTAR_DIR \
  -e MYSQL_TEST_USER=$MYSQL_TEST_USER \
  -e MYSQL_TEST_PASSWD=$MYSQL_TEST_PASSWD \
  -e MYSQL_TEST_PORT=$MYSQL_TEST_PORT \
  -e MYSQL_TEST_HOST=$MYSQL_TEST_HOST \
  -e MYSQL_TEST_DB=$MYSQL_TEST_DB \
  -e MYSQL_TEST_VERBOSE=$MYSQL_TEST_VERBOSE \
  -e MARIADB_CC_TEST=$MARIADB_CC_TEST \
  -e MYSQL_TEST_TLS=$MYSQL_TEST_TLS \
  -e MYSQL_TEST_SSL_PORT=$MYSQL_TEST_SSL_PORT \
  -e SIDECAR_NAME=$SIDECAR_NAME \
  -e MARIADB_TLS_ONLY_TEST_ENV_VAR=$MARIADB_TLS_ONLY_TEST_ENV_VAR \
  -e MARIADB_TLS_DEBUG_ENV_OVERRIDE=$MARIADB_TLS_DEBUG_ENV_OVERRIDE \
  -e MARIADB_TLS_DEBUG_OPENSSL=$MARIADB_TLS_DEBUG_OPENSSL \
  -v $VOLUME_NAME:$VOLUME_MOUNT_POINT \
  -w $VOLUME_MOUNT_POINT \
  --network $NETWORK_NAME \
  --rm \
