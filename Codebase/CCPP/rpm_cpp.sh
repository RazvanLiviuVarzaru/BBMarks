...
export GIT_REPO=https://github.com/RazvanLiviuVarzaru/mariadb-connector-cpp.git
export GIT_BRANCH=cpp-1.0
export GIT_COMMIT=ee02ec224dae24e454b9cc6702ff6a8531259c30
...
require_rhel_host_if_rhel_image() {
  ...
  if [[ "$IMAGE" == *rhel* ]]; then
    if ! host_is_rhel; then
      ...
      echo "You need an activated RHEL host for building C/CPP on RHEL." >&2
      exit 1
    fi
...
docker run \
  ...
  bash -ec '
    ...
    baseurl="https://rpm.mariadb.org/10.11/$url_path"
...
EOF
    ...
    su - buildbot -c "
      ...
      mkdir -p $RPM_DIR
      cd $RPM_DIR
      cmake -DCMAKE_C_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DRPM=On -DUSE_SYSTEM_INSTALLED_LIB=ON -DCPACK_GENERATOR=RPM -DWITH_SSL=OPENSSL -DCMAKE_BUILD_TYPE=RelWithDebInfo -DMARIADB_LINK_DYNAMIC=On -DPACKAGE_PLATFORM_SUFFIX=$HOSTNAME $SOURCE_DIR
